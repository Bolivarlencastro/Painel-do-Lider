...
interface MandatoryCoursesTableProps {
...
const DueDate: React.FC<{ dueDate: string }> = ({ dueDate }) => {
  ...
  let urgencyText = `Vence em ${diffDays} dias`;

  if (diffDays < 0) {
    colorClass = 'text-red-600 dark:text-red-400 font-semibold';
    urgencyText = `Vencido há ${Math.abs(diffDays)} dias`;
  } else if (diffDays === 0) {
    colorClass = 'text-red-600 dark:text-red-400 font-semibold';
  ...
  return <span className={`text-sm ${colorClass}`}>{urgencyText}</span>;
};
...
export const MandatoryCoursesTable: React.FC<MandatoryCoursesTableProps> = ({ enrollments, members, onViewDetails, onViewAll, isTeamEmpty }) => {
  ...
  const { displayItems, totalCount } = React.useMemo(() => {
    const normativeEnrollments = enrollments.filter(e => e.isNormativa && e.dueDate && e.progress < 100);
    const sortedItems = normativeEnrollments
            ...
            {displayItems.length > 0 ? (
                 <ul className="divide-y divide-gray-200 dark:divide-gray-700/60">
                    {displayItems.map((item) => {
                      ...
                      if (!member || !course || !item.dueDate) return null;

                      return (
                        <li key={`${item.memberId}-${item.courseId}`} className="p-3 min-h-[6rem] flex items-center gap-4 hover:bg-gray-50 dark:hover:bg-gray-700/40 rounded-lg transition-colors cursor-pointer" onClick={(e) => handleViewDetails(e, member)}>
                 ...
                 <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-gray-400 p-4">
                    <p>{isTeamEmpty ? "Não há liderados na equipe para exibir dados." : "Nenhum item de atenção imediata. A equipe está em dia!"}</p>
                </div>
            )}
        </div>
    </div>
