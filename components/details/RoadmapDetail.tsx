...
import { DevelopmentPlan, TeamMember, Course, Trail, Pulse, SelectedItem, MemberStatus, ContentRef } from '../../types';
...
// Helper to get a member's progress on an entire plan
...
const getTeamProgressOnContent = (contentRef: ContentRef, memberIds: string[], allMembers: TeamMember[]): number => {
...
export const PlanDetail: React.FC<PlanDetailProps> = ({ plan, allMembers, allCourses, allTrails, allPulses, onItemClick }) => {
    const [activeTab, setActiveTab] = useState<DetailTab>('liderados');

    const assignedMembers = useMemo(() => allMembers.filter(m => plan.memberIds.includes(m.id)), [allMembers, plan]);
    const planContent = useMemo(() => {
      ...
      return plan.content.map(c => ({
        ...c,
        name: contentMap[c.type].get(c.id) || 'Conteúdo não encontrado'
      }));
    }, [plan, allCourses, allTrails, allPulses]);

    const viewedProgress = plan.memberIds.length > 0 ? Math.round((plan.viewedByMemberIds.length / plan.memberIds.length) * 100) : 0;

    const renderContent = () => {
        switch (activeTab) {
            case 'liderados':
                return <MemberList members={assignedMembers} plan={plan} onItemClick={onItemClick} />;
            case 'conteudos':
                return <ContentList contentItems={planContent} memberIds={plan.memberIds} allMembers={allMembers} onItemClick={onItemClick} />;
        }
    };
        ...
        <div className="space-y-6">
            <div className="p-4 bg-gray-100 dark:bg-gray-700/30 rounded-lg">
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-2 text-sm text-gray-500 dark:text-gray-400">
                    ...
                    {plan.dueDate && <div className="flex items-center gap-2"><Icon name="event_busy" size="sm"/> <span>Prazo: {new Date(plan.dueDate).toLocaleDateString()}</span></div>}
                    <div className="flex items-center gap-2"><Icon name="group" size="sm"/> <span>{plan.memberIds.length} Liderados</span></div>
                    <div className="flex items-center gap-2" title={`${plan.viewedByMemberIds.length} de ${plan.memberIds.length} liderados visualizaram`}><Icon name="visibility" size="sm"/> <span>Visualização: {viewedProgress}%</span></div>
                </div>
            </div>
            ...
            <div>
                <div className="border-b border-gray-200 dark:border-gray-700">
                    <nav className="-mb-px flex space-x-6 overflow-x-auto" aria-label="Tabs">
                        ...
                        <TabButton<DetailTab> id="conteudos" label={`Conteúdos (${planContent.length})`} icon="checklist" activeTab={activeTab} setActiveTab={setActiveTab} />
                    </nav>
                </div>
                <div className="pt-6">
                    {renderContent()}
...
}> = ({ members, plan, onItemClick }) => {
    if (members.length === 0) return <div className="text-center py-8 text-gray-500 dark:text-gray-400">Nenhum liderado neste plano.</div>;

    return (
        <ul className="space-y-3">
            {members.map(member => {
                const progress = getMemberProgressOnPlan(member, plan);
                const hasViewed = plan.viewedByMemberIds.includes(member.id);
                return (
...
const ContentList: React.FC<{
    contentItems: (ContentRef & { name: string })[];
    memberIds: string[];
    allMembers: TeamMember[];
    onItemClick: (type: SelectedItem['type'], data: any) => void;
}> = ({ contentItems, memberIds, allMembers, onItemClick }) => {
    if(contentItems.length === 0) return <div className="text-center py-8 text-gray-500 dark:text-gray-400">Nenhum conteúdo neste plano.</div>;
